...
| ~~`OSC 8`~~ | ~~ハイパーリンク~~ | **✅ 実装済み（2026-09-13）**。id-interner モデル（`HyperlinkInterner`）で `HyperlinkId` を発行し、GC はしない（id は再利用しない）。target の長さやバイト範囲の検証もしない。開いているリンクは Pen ではなく `DeviceState::active_hyperlink` に 1 つだけ持ち、`DeviceState::print` が IRM・DECAWM と一緒に `PrintOptions` にまとめて `Screen::print` へ渡し、`Pen::stamp` がセルに書く（2026-09-14、kitty と同じモデル）。`SGR`（`SGR 0` を含む）は触らない。`DECSC` は保存せず `DECRC` でも戻らない。47/1047/1049 の切替はどちら向きでも閉じる。`DECSTR` と `RIS` も閉じるが、`RIS` でも interner 自体は残す。**既に代替画面にいるときの 47/1047/1049 の SET はリンクを閉じない**（2026-09-14 判断）。代替画面で落ちたプログラムのリンクが次の全画面プログラムに漏れる経路は残るが、プロンプトや主画面での漏れはどのみち防げず、10 端末中 8 が同じく閉じない（閉じる VTE と Windows Terminal も漏れ対策ではなく副作用）。**多数派からの逸脱は 2 点**（2026-09-14 に 11 端末のソースを調査）: DECRC でリンクを戻す端末は OSC 8 実装 10 端末中 6（WezTerm・VTE・tmux・contour・Windows Terminal・Alacritty）、1049 往復後に主画面のリンクが残る端末は 10 中 8 で、閉じるのは kitty と Ghostty だけ。仕様（egmontkob の gist）はどちらも規定しない | nvim。レンダラ側の受け皿と接続済み |
...
8. ~~**OSC 52**~~ **書き込み方向は完了（2026-09-13）**。残るのは **`?`（読み出し）**。
   ...
   見送ったのは配線が無いからではなく、読み出しを持たない（書き込みとクリアだけ）ため。ただし不正な OSC 52 でクリアするようになったので（2026-09-14）、`cat` した出力でユーザーのクリップボードが消える経路ができた。ゲートを入れるかは、この点も含めて判断する。OSC のサイズ上限も同様に見送り（`std` 下の vtparse は
   `Vec<u8>` で上限が無く、これは OSC 52 固有ではなく OSC 0/2/7 も含むパーサ全体の話。
   alacritty の vte も `std` では同じく無制限）。ほかに **DECLRMM/DECSLRM**、**1015**。
