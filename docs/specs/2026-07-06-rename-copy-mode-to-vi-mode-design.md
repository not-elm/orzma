...
| The `TmuxMouseEffect` mouse copy-drag family: `CopySelection` (`src/input/mouse/button/tmux/effect.rs:45`, a **second** `CopySelection` distinct from the config enum), plus `BeginCopyDrag` / `ExtendCopyDrag`, and the `copy-drag` comments describing them | Click-drag-to-copy mouse interaction (its apply arm triggers `TerminalSelectionCopy`, `apply.rs:127`), not the vi-mode keyboard concept. A blind `CopySelection → ViModeSelection` would corrupt it — hence the config enum is renamed symbol-scoped (above) and the whole mouse family is kept as one coherent unit |
