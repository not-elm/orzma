...
pub struct Cursor {
    ...
    pub blinking: bool,
    /// True when the application wants the cursor drawn, which DECTCEM
    /// (`CSI ? 25 h` / `l`) alone decides. The derived `Default` leaves
    /// this `false`, so production code should not reach for
    /// `Cursor::default()` to mean "visible".
