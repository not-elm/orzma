//! Session-scoped and global environment commands used to propagate `$ORZMA_SOCK`
...
mod tests {
    ...
    fn set_environment_in_session_targets_named_session() {
        assert_eq!(
            ...
            .into_raw_command(),
            "set-environment -t work ORZMA_SOCK /tmp/ctl.sock"
        );
    }
    ...
    fn set_environment_in_session_quotes_session_and_value() {
        assert_eq!(
            ...
            .into_raw_command(),
            "set-environment -t 'my work' ORZMA_SOCK '/tmp/a b/ctl.sock'"
        );
    }
    ...
    fn set_environment_global_emits_dash_g_flag() {
        assert_eq!(
            ...
            .into_raw_command(),
            "set-environment -g ORZMA_SOCK /tmp/ctl.sock"
        );
    }
    ...
    fn set_environment_global_quotes_value_with_spaces() {
        assert_eq!(
            ...
            .into_raw_command(),
            "set-environment -g ORZMA_SOCK '/tmp/a b/ctl.sock'"
        );
    }
    ...
    fn unset_environment_global_emits_dash_gu_flag() {
        assert_eq!(
            UnsetEnvironmentGlobal { key: "ORZMA_SOCK" }.into_raw_command(),
            "set-environment -gu ORZMA_SOCK"
        );
    }
}
