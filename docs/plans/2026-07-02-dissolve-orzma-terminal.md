# Dissolve `orzma_terminal` into `src/` — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Delete `crates/orzma_terminal` and move its ~1,150 lines into the binary (`src/`), following the input → action architecture, with zero behavior change.

**Architecture:** The crate is emptied leaf-first so every task leaves the whole workspace compiling and tested: first sever the one library consumer (`orzma_webview`), then move the action layer (`paste`, mouse apply events) into a new `src/action/terminal/` domain, then the shared `Clipboard`, then the Default-mode `exit`/`layout` systems, and finally the `OrzmaTerminal` marker + spawn types — at which point the crate is deleted. Type and event names are preserved; the bulk of the diff is `use` path rewrites.

**Tech Stack:** Rust edition 2024 (toolchain 1.95), Bevy 0.18 ECS (EntityEvent + observer idiom), `arboard`, `open`, `anyhow`.

**Spec:** `docs/specs/2026-07-02-dissolve-orzma-terminal-design.md` (reviewed and approved).
...
### Task 1: Re-key `orzma_webview` GC on `TerminalHandle` and drop the `orzma_terminal` dependency

**Files:**
- Modify: `crates/orzma_webview/src/control_plane.rs` (imports at :16, `gc_despawned_surfaces` at :402-424, test at :849-884)
- Modify: `crates/orzma_webview/Cargo.toml` (drop the `orzma_terminal` path dep at :19)
- Modify: `src/mode/default.rs:89-91` (stale `// NOTE:` documenting the gc keying)
- Test: in-file `gc_tests` module of `control_plane.rs`

**Interfaces:**
- Consumes: `TerminalHandle` and `TerminalHandle::detached(cols, rows)` from `orzma_tty_engine` (already a dependency of `orzma_webview`).
- Produces: `orzma_webview` no longer references `orzma_terminal` in any form — later tasks can move/delete the crate without touching `orzma_webview` again.

- [ ] **Step 1: Change the gc test to spawn `TerminalHandle` instead of `OrzmaTerminal` (failing test)**

In `crates/orzma_webview/src/control_plane.rs`, inside `mod gc_tests`, replace:
...
```rust
use orzma_terminal::OrzmaTerminal;
```
...
Leave `gc_despawned_surfaces` itself untouched for now. It still keys on `RemovedComponents<OrzmaTerminal>` — which no longer compiles because the import is gone. To keep this a true red/green cycle without a broken build, do the signature change and the test change in the same commit but verify the test-level behavior in Step 2 by reasoning: if you prefer a strictly compiling red state, temporarily keep both imports (`use orzma_terminal::OrzmaTerminal;` AND `use orzma_tty_engine::TerminalHandle;`), run Step 2, then finish Step 3.

- [ ] **Step 2: Run the gc test to verify it fails**

Run: `cargo test -p orzma_webview gc_purges_registrations_when_owner_surface_despawns`

Expected: FAIL — the assertion `"despawning the owner surface purges its registrations"` trips, because the spawned surface never carried `OrzmaTerminal`, so `RemovedComponents<OrzmaTerminal>` never sees the despawn and the registration survives.

- [ ] **Step 3: Re-key `gc_despawned_surfaces` on `TerminalHandle`**

In `crates/orzma_webview/src/control_plane.rs`:

1. Ensure the `use` block has `use orzma_tty_engine::TerminalHandle;` and NOT `use orzma_terminal::OrzmaTerminal;` (remove the temporary dual import if you kept one).
...
- [ ] **Step 5: Drop the `orzma_terminal` dependency from `orzma_webview`**

In `crates/orzma_webview/Cargo.toml`, delete the line:

```toml
orzma_terminal = { path = "../orzma_terminal" }
```

Run: `grep -rn "orzma_terminal" crates/orzma_webview/` — Expected: no matches.
...
```rust
            ...
            // RemovedComponents<OrzmaTerminal> (never added on the error path),
...
Run: `cargo build && cargo test -p orzma_webview`
Expected: build OK, all `orzma_webview` tests pass.

```bash
git add crates/orzma_webview/ src/mode/default.rs
git commit -m "refactor(webview): key surface gc on TerminalHandle, drop orzma_terminal dep"
```
...
- Modify: `crates/orzma_terminal/src/lib.rs` (drop the `action` module)
- Delete: `crates/orzma_terminal/src/action.rs`
- Test: in-file test moves with `paste.rs`

**Interfaces:**
- Consumes (temporarily, until Tasks 4/6): `orzma_terminal::{Clipboard, build_paste_bytes, OrzmaTerminal}`; `orzma_tty_engine::{Coalescer, PtyHandle, TerminalHandle}`.
...
Move the content of `crates/orzma_terminal/src/action.rs` with visibility demoted and imports adjusted:
...
use bevy::prelude::*;
use orzma_terminal::{Clipboard, OrzmaTerminal, build_paste_bytes};
...
fn on_paste(
    ...
    mut terminals: Query<(&mut TerminalHandle, &mut PtyHandle, &mut Coalescer), With<OrzmaTerminal>>,
) {
...
(The test previously registered `OrzmaActionPlugin`; `PastePlugin` is its exact replacement.)
...
| File | Old | New |
|---|---|---|
| `src/input/keyboard.rs:14` | `use orzma_terminal::{OrzmaTerminal, PasteAction};` | `use crate::action::terminal::PasteAction;` and `use orzma_terminal::OrzmaTerminal;` (two lines, merged into the existing single import block in alphabetical position) |
| `src/input/default_mode.rs:29` | `use orzma_terminal::{OrzmaTerminal, PasteAction};` | same split as above |

- [ ] **Step 5: Remove the `action` module from the crate**

In `crates/orzma_terminal/src/lib.rs`:
- delete `mod action;`
- delete `use crate::action::OrzmaActionPlugin;`
- delete `pub use action::PasteAction;`
- change the plugin tuple `(ExitPlugin, LayoutPlugin, OrzmaActionPlugin, OrzmaMousePlugin)` to `(ExitPlugin, LayoutPlugin, OrzmaMousePlugin)`

Then: `git rm crates/orzma_terminal/src/action.rs`

- [ ] **Step 6: Build, test, commit**

Run: `cargo build && cargo test`
Expected: all green. In particular `paste_action_on_entity_without_terminal_does_not_panic` now runs from the binary's test set (`cargo test -p orzma paste_action`).

```bash
git add src/action.rs src/action/terminal.rs src/action/terminal/ src/input/keyboard.rs src/input/default_mode.rs crates/orzma_terminal/
...
Move `TerminalMouseWrite` + `on_terminal_mouse_write` + the `detached_write_event_forwards_bytes` test from `crates/orzma_terminal/src/mouse.rs`:
...
use bevy::prelude::*;
use orzma_terminal::{Clipboard, OrzmaTerminal};
use orzma_tty_engine::{Coalescer, Point, PtyHandle, SelectionType, Side, TerminalHandle};
...
Move `TerminalOpenUri` (:94-102), `on_terminal_open_uri` (:315-319), and fold in `try_open_uri` from `crates/orzma_terminal/src/hyperlink.rs` as a private helper:
...
| File | Old | New |
|---|---|---|
| `src/input/mouse.rs:17-20` | `use orzma_terminal::{\n    OrzmaTerminal, TerminalMouseWrite, TerminalOpenUri, TerminalSelectionClear,\n    TerminalSelectionCopy, TerminalSelectionStart, TerminalSelectionUpdate,\n};` | `use crate::action::terminal::{\n    TerminalMouseWrite, TerminalOpenUri, TerminalSelectionClear, TerminalSelectionCopy,\n    TerminalSelectionStart, TerminalSelectionUpdate,\n};` plus `use orzma_terminal::OrzmaTerminal;` |
| `src/input/mouse/wheel.rs:19` | `use orzma_terminal::{TerminalMouseWrite, TerminalViewportScroll};` | `use crate::action::terminal::{TerminalMouseWrite, TerminalViewportScroll};` |
| `src/input/tmux/forward.rs:6` | `use orzma_terminal::TerminalForwardInput;` | `use crate::action::terminal::TerminalForwardInput;` |

- [ ] **Step 8: Remove the `mouse` and `hyperlink` modules from the crate**

In `crates/orzma_terminal/src/lib.rs`:
- delete `mod hyperlink;` and `mod mouse;`
- delete `use crate::mouse::OrzmaMousePlugin;`
- delete the `pub use mouse::{ ... };` re-export block
- change the plugin tuple `(ExitPlugin, LayoutPlugin, OrzmaMousePlugin)` to `(ExitPlugin, LayoutPlugin)`

Then: `git rm crates/orzma_terminal/src/mouse.rs crates/orzma_terminal/src/hyperlink.rs`

- [ ] **Step 9: Build, test, commit**

Run: `cargo build && cargo test`
Expected: green; the three moved tests now run in `orzma` (`cargo test -p orzma detached_write detached_selection viewport_scroll`).

```bash
git add src/action/terminal.rs src/action/terminal/ src/input/mouse.rs src/input/mouse/wheel.rs src/input/tmux/forward.rs crates/orzma_terminal/
...
- Modify: `crates/orzma_terminal/src/lib.rs` (drop the `clipboard` module)
- Delete: `crates/orzma_terminal/src/clipboard.rs`
...
Copy `crates/orzma_terminal/src/clipboard.rs` verbatim (261 lines including tests), then apply exactly these changes:
...
| File | Old | New |
|---|---|---|
| `src/action/terminal/paste.rs` | `use orzma_terminal::{Clipboard, OrzmaTerminal, build_paste_bytes};` | `use crate::clipboard::{Clipboard, build_paste_bytes};` + `use orzma_terminal::OrzmaTerminal;` |
| `src/action/terminal/selection.rs` | `use orzma_terminal::{Clipboard, OrzmaTerminal};` | `use crate::clipboard::Clipboard;` + `use orzma_terminal::OrzmaTerminal;` |
| `src/action/terminal/mouse_write.rs` (tests) | `use orzma_terminal::Clipboard;` | `use crate::clipboard::Clipboard;` |
| `src/ui/copy_mode.rs:15` | `use orzma_terminal::Clipboard;` | `use crate::clipboard::Clipboard;` |
| `src/mode/tmux/copy_mode.rs:20` | `use orzma_terminal::Clipboard;` | `use crate::clipboard::Clipboard;` |
| `src/input/tmux/input.rs:47` | `use orzma_terminal::{Clipboard, build_paste_bytes};` | `use crate::clipboard::{Clipboard, build_paste_bytes};` |
| `src/action/vi/default_mode.rs:10` | `use orzma_terminal::Clipboard;` | `use crate::clipboard::Clipboard;` |
| `src/input/mouse/button.rs:491` (test) | `use orzma_terminal::Clipboard;` | `use crate::clipboard::Clipboard;` |
| `src/input/mouse/wheel.rs:273` (test) | `use orzma_terminal::{Clipboard, OrzmaTerminal};` | `use crate::clipboard::Clipboard;` + `use orzma_terminal::OrzmaTerminal;` |

After the table, run `grep -rn "orzma_terminal::Clipboard\|orzma_terminal::{Clipboard\|Clipboard, build_paste_bytes" src/` and fix any site the table missed the same way. Expected: no `orzma_terminal` clipboard imports remain.
...
/// `Clipboard` resource exists (idempotent — `OrzmaActionPlugin` already
...
In `crates/orzma_terminal/src/lib.rs`: delete `mod clipboard;` and `pub use clipboard::{Clipboard, build_paste_bytes};`.
Then: `git rm crates/orzma_terminal/src/clipboard.rs`

- [ ] **Step 7: Build, test, commit**

Run: `cargo build && cargo test`
Expected: green; clipboard tests run in `orzma` (`cargo test -p orzma build_paste_bytes`).

```bash
git add src/clipboard.rs src/main.rs src/action/ src/ui/copy_mode.rs src/mode/tmux/copy_mode.rs src/input/ crates/orzma_terminal/
...
- Modify: `crates/orzma_terminal/src/lib.rs` (drop both modules)
- Delete: `crates/orzma_terminal/src/exit.rs`, `crates/orzma_terminal/src/layout.rs`
- Test: in-file tests move with each file

**Interfaces:**
- Consumes (temporarily): `orzma_terminal::{OrzmaTerminal, cells_for}`; `orzma_tty_engine::{Coalescer, PtyHandle, TerminalChildExit, TerminalHandle}`; `orzma_tty_renderer::TerminalCellMetricsResource`.
...
Move `crates/orzma_terminal/src/exit.rs` verbatim (including its test module), with these changes:
- imports: `use orzma_terminal::OrzmaTerminal;` replaces `use crate::spawn::OrzmaTerminal;` (and the test's `use crate::spawn::OrzmaTerminal;` becomes `use orzma_terminal::OrzmaTerminal;` — or is covered by `use super::*`).
...
Move `crates/orzma_terminal/src/layout.rs` verbatim (including tests), with these changes:
- imports: `use crate::spawn::{OrzmaTerminal, cells_for};` → `use orzma_terminal::{OrzmaTerminal, cells_for};`
...
Everything else (the `OrzmaLastSize` resource, `reset_last_size` observer, the `run_if` chain in `build`, both tests) moves unchanged.
...
Then: `git rm crates/orzma_terminal/src/exit.rs crates/orzma_terminal/src/layout.rs`
...
Run: `cargo build && cargo test`
Expected: green; `cargo test -p orzma child_exit_sends_app_exit orzma_terminal_spawn_resets_last_size` pass in the binary.

```bash
git add src/mode/default.rs src/mode/default/ crates/orzma_terminal/
git commit -m "refactor(mode): move shell exit + window-fill layout into src/mode/default/"
```

---

### Task 6: Move the marker + spawn types; delete `crates/orzma_terminal`
...
- Modify: root `Cargo.toml` (drop `orzma_terminal` dep, add `anyhow`)
- Delete: `crates/orzma_terminal/` (entire directory; workspace `members` uses the `crates/*` glob, so no members edit is needed)
...
**Interfaces:**
- Consumes: `orzma_tty_engine::{SpawnOptions, TerminalBundle}`, `orzma_tty_renderer::material::TerminalUiMaterial`, `orzma_tty_renderer::prelude::TerminalRenderBundle`, `anyhow`.
- Produces: `crate::surface::{OrzmaTerminal, SurfacePlugin}`; `crate::surface_geom::cells_for(w_px: u32, h_px: u32, cell_w: f32, cell_h: f32) -> (u16, u16)`; `crate::mode::default::spawn::{OrzmaSpawnOptions, OrzmaTerminalBundle, OrzmaTerminalConfig}` (pub(crate)); `DefaultSpawnPlugin { shell: Option<String> }` (pub(super)); `DefaultModePlugin` gains a `pub config_shell: Option<String>` field.
...
Move the remainder of `crates/orzma_terminal/src/spawn.rs` (bundle, options, config, `resolve_shell`, shell-resolution tests):
...
2. Import rewrite: `use orzma_terminal::{OrzmaSpawnOptions, OrzmaTerminalBundle, OrzmaTerminalConfig};` → `use crate::mode::default::spawn::{OrzmaSpawnOptions, OrzmaTerminalBundle, OrzmaTerminalConfig};`
3. Give the plugin its field and register the spawn plugin (also fix the now-stale doc sentence "`OrzmaTerminalPlugin` must be added first (it inserts the `OrzmaTerminalConfig` this reads)" → "`DefaultSpawnPlugin` (registered below) inserts the `OrzmaTerminalConfig` this reads."):
...
4. In the in-file tests' `build_app`, `OrzmaTerminalConfig` now comes from `spawn`, and `DefaultModePlugin` needs the field. Replace:
...
- Remove `use orzma_terminal::OrzmaTerminalPlugin;`.
- Add `mod surface;` (Step 1) and `use crate::surface::SurfacePlugin;`.
- In the plugin list, replace:

```rust
            OrzmaTerminalPlugin {
                config_shell: pre_configs.orzma.shell.clone(),
            },
            DefaultModePlugin,
```
...
Mechanical sweep — for every file below, change `use orzma_terminal::OrzmaTerminal;` to `use crate::surface::OrzmaTerminal;` (both top-of-file and test-module-local `use` statements):
...
| File | Old | New |
|---|---|---|
| `src/mode/tmux/render.rs:11` | `use orzma_terminal::{OrzmaTerminal, cells_for};` | `use crate::surface::OrzmaTerminal;` + `use crate::surface_geom::cells_for;` |
| `src/mode/tmux/adopt.rs:21` | `use orzma_terminal::cells_for;` | `use crate::surface_geom::cells_for;` |
| `src/mode/default/exit.rs` (Task 5) | `use orzma_terminal::OrzmaTerminal;` | `use crate::surface::OrzmaTerminal;` |
| `src/mode/default/layout.rs` (Task 5) | `use orzma_terminal::{OrzmaTerminal, cells_for};` | `use crate::surface::OrzmaTerminal;` + `use crate::surface_geom::cells_for;` |

- [ ] **Step 7: Swap the render-test plugin and sweep the stale `orzma_terminal` comments**

In `src/mode/tmux/render.rs`:

1. Test import (:723): `use orzma_terminal::OrzmaTerminalPlugin;` → `use crate::surface::SurfacePlugin;`
2. Three call sites (:856, :913, :1648): `app.add_plugins(OrzmaTerminalPlugin { config_shell: None });` → `app.add_plugins(SurfacePlugin);` (the tests only need the render-injection observer; the config resource and layout/exit systems were never exercised by them — if a test fails on a missing `OrzmaTerminalConfig`, add `app.insert_resource(crate::mode::default::spawn::OrzmaTerminalConfig { shell: None });` — not expected).
3. Doc comment (:177): "The `On<Add, OrzmaTerminal>` observer in `orzma_terminal` injects the" → "The `On<Add, OrzmaTerminal>` observer in `crate::surface` injects the"

In `src/input/hyperlink.rs` module doc (:12-13): replace

```rust
//! `crate::input::mouse` (deciders `decide_button`/`resolve_button_event`);
//! `orzma_terminal` keeps only the apply observer (`on_terminal_open_uri` →
//! `try_open_uri`).
```
...
|---|---|---|
| `src/input/tmux/forward.rs:1` | ``//! Routes `orzma_terminal`'s `TerminalForwardInput` (backend-bound bytes from the`` | ``//! Routes `crate::action::terminal`'s `TerminalForwardInput` (backend-bound bytes from the`` |
| `src/input/default_mode.rs:7` | ``//! paste are owned by `orzma_terminal`'s dispatcher and `PasteAction`.`` | ``//! paste are owned by the keyboard dispatcher and `crate::action::terminal::PasteAction`.`` |
| `src/input/tmux/gate.rs:76` | ``// both resize the pane and start an `orzma_terminal` selection — the`` | ``// both resize the pane and start a local terminal selection — the`` |
| `src/input/tmux/input.rs:586` | ``/// `orzma_terminal`'s `WheelAction` resolves to a `ScrollViewport` that is a`` | ``/// the host's `WheelAction` resolves to a `ScrollViewport` that is a`` |
| `src/input/tmux/input.rs:868` | `// to orzma_terminal — forward_wheel_to_tmux emits no send-keys for it.` | `// to the local terminal — forward_wheel_to_tmux emits no send-keys for it.` |
| `src/input/tmux/mouse.rs:166` | ``/// `orzma_terminal`). Multi-click (≥2) on a pane in copy mode enters`` | ``/// the local terminal path). Multi-click (≥2) on a pane in copy mode enters`` |

Test function names containing `orzma_terminal` (`src/input/ime.rs:929`, `src/mode/tmux/render.rs:1644`) are identifiers describing the marker component, not crate references — leave them.

- [ ] **Step 8: Delete the crate and fix the manifests**

```bash
git rm -r crates/orzma_terminal
```

In the root `Cargo.toml` `[dependencies]`:
- delete `orzma_terminal = { path = "crates/orzma_terminal" }`
...
```bash
grep -rn "orzma_terminal" src/ crates/ Cargo.toml | grep -v "_orzma_terminal"
```
...
```bash
git add -A
git commit -m "refactor: dissolve orzma_terminal crate into src/ (surface, spawn, cells_for)"
```
...
**Files:**
- Modify: whatever `just fix-lint` touches; `CLAUDE.md` only if it still references `orzma_terminal` (the current crate map already omits it)
...
Run: `grep -rn "orzma_terminal" CLAUDE.md docs/ --include="*.md" | grep -v docs/specs/ | grep -v docs/plans/`
...
```bash
git add -A
git commit -m "chore: lint sweep after orzma_terminal dissolution"
```
