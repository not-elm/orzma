//! Shared CEF screencast WebSocket helper.
//!
//! Both `/browser/ws` (Browser-kind) and `/extension/cef/ws` (Extension-kind)
//! stream `BrowserServerMsg` frames from a per-activity `FrameRing` and accept
//! `BrowserClientMsg::Subscribe` / `Resize` / `Input` / `Navigate` /
//! `NavigateHistory` from the client. This module owns the wire-format loop
//! and the origin/kind guards; route modules pick the expected kind and
//! delegate here.
...
/// Validate origin and activity kind, then upgrade to a WebSocket bound to the
/// CEF `FrameRing` registered for `aid`. Closes the upgraded socket
/// immediately if no ring is registered for that activity.
pub(super) async fn cef_screencast_ws(
    state: AppState,
    wid: WindowId,
    pid: PaneId,
    aid: ActivityId,
    expected: ActivityKindDiscriminant,
