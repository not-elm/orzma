...
- [x] OS から得たディレクトリは、存在するだけでなく入れる（検索権限がある）ことも確かめてから採用し、入れなければ次の候補に進む。
- [ ] OSC 7 で報告されたディレクトリと spawn 時のディレクトリは確かめないので、存在しても実行（検索）権限の無いディレクトリだと spawn の chdir が失敗し、分割そのものが `SpawnFailed` になる。入れるかどうかを確かめてから採用するか、失敗時にホームディレクトリで起動し直す。
