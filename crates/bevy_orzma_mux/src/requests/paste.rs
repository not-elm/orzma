//! `RequestTtyPaste`: text the host UI asks a terminal entity to receive
//! as if typed.
...
/// Registers the [`RequestTtyPaste`] apply observer.
...
fn apply_paste(e: On<RequestTtyPaste>, mut terms: Query<&mut OrzmaTtyHandle>) {
...
mod tests {
    ...
    fn app_with_terminal() -> (App, Entity, CaptureSink) {
        ...
        let (handle, sink) = OrzmaTtyHandle::detached(80, 24);
    ...
    fn paste_honours_live_bracketed_paste_mode() {
        for (text, expected) in [
            ...
            let mut app = App::new();
            app.add_plugins(PastePlugin);
            let (mut handle, sink) = OrzmaTtyHandle::detached(80, 24);
    ...
    fn paste_targets_only_the_addressed_terminal() {
        ...
        let (other_handle, other_sink) = OrzmaTtyHandle::detached(80, 24);
