...
A keyboard-driven TUI browser for ozmux panes — a companion app built with the
[`ratatui-ozma`](../../sdk/ratatui-ozma) SDK.

## Overview

ozbrowser runs inside an ozmux pane and loads a remote URL in an embedded
...
ozbrowser is installed from source alongside the other companion apps:
...
ozbrowser must be launched **inside an ozmux pane**. Run anywhere else, it exits
with:

```
ozbrowser: not inside an ozmux pane: OZMA_SOCK is unset. Run ozbrowser inside an ozmux pane.
```
...
ozbrowser's keyboard model and link-hint workflow are inspired by
...
ozbrowser ships an independent implementation rather than Vimium source.
