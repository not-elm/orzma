...
6. ~~**OSC 4**~~ ~~**OSC 10/11**~~ **完了（OSC 4 は 2026-09-12、OSC 10/11 は 2026-09-13。いずれも `?` 問い合わせとリセットを含む）** → 残るのは **OSC 12 / 112**（カーソル色）で、これは §2 の `OSC 12` 行のとおりレンダラ側の作業を伴う。以下はこの項目を書いた時点の見通しで、**実際にはそのとおりにしなかった** — `PaletteRequest` を広げる代わりに `DynamicColorRequest` を別モジュールに置いた（理由は §1-B の `OSC 10/11` 行）。`frame.rs` の `palette` フィールドの TODO は解消済み。OSC 4 で入れた `PaletteRequest` を広げて扱う。RIS での復帰は `Palette::reset`（全色を既定値へ戻す）が既に賄うので、ハンドラ側は `Palette` の `foreground` / `background` を書くのと、full repaint の staging（`frame.rs` の `palette` フィールドの TODO）を足すだけでよい。なお `OSC 104` は xterm-ctlseqs.pdf のとおりインデックス表だけを戻す（`Palette::reset_all_indexed`）ので、そちらに前景/背景を巻き込まないこと。
