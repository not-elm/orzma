# Design: `docs/orzma_webview_protocol.md` — Orzma Webview Protocol specification

## Goal

Write a new public document, `docs/orzma_webview_protocol.md`, that fully
specifies the Orzma Webview protocol so a developer can implement a client in
...
- **Depth:** a complete, language-agnostic **wire specification** — exact OSC
  ...
  `orzma://` origin rules, and the page-side `window.orzma` API.
- **Out of scope:** orzma's internal ECS/CEF/Bevy mechanics (anchors, overlay
...
- What the protocol is: a local program running in an orzma pane registers
...
- The three surfaces: (1) the **control socket** (register/manage + back-channel
  routing), (2) **OSC 5379** (mount/unmount), (3) the **`window.orzma`** page
  bridge.
- The actors: **registering program** ↔ **orzma host** ↔ **webview page**.
...
- One ASCII sequence/data-flow diagram showing: connect+hello → register → OSC
  mount → page loads `orzma://<handle>/` → `window.orzma.call` → host `call` →
  program `reply` → `window.orzma` Promise resolves → OSC unmount / disconnect.
...
- **Discovery:** the program reads two env vars injected into its PTY:
  - `$ORZMA_SOCK` — absolute path to the control socket. Clients MUST use this
    value; do not reconstruct the path.
  - `$ORZMA_TOKEN` — the per-pane handshake token (opaque string, currently of
    the form `orzma:<bits>`; treat as opaque).
- **Peer authentication:** the listener checks the connecting peer's UID equals
  the orzma process UID and silently drops the connection otherwise. (Observable
  effect: only same-user processes can connect.)
- **Handshake:** the **first** line MUST be `hello` carrying `$ORZMA_TOKEN`. If
...
| `emit` | `handle`, `event`, `payload` | Push an event to the handle's pages (`window.orzma.on`). |
...
- **Register kinds:**
  - `dir` — `root` (absolute path; must exist and be a directory), `entry`
    ...
    Served at `orzma://<handle>/`.
  - `inline` — `html` (full document, ≤ **4 MiB**), `interactive`,
    `forward_keys`, `preload`. Served at `orzma://<handle>/index.html`.
  - `url` — `url` (`http`/`https` only), `interactive`, `bridge` (default
    `false` — opt-in to `window.orzma`), `forward_keys`, `preload`. Loaded
    directly; no `orzma://` origin.
...
| --- | --- | --- |
| `call` | `handle`, `reqId`, `method`, `params` | A page `window.orzma.call(method, params)`. Respond with `reply` carrying the same `reqId`. |
...
- **Direction asymmetry to call out explicitly:** a page's
  `window.orzma.emit(event, …)` arrives at the program as `op: "event"`
  (inbound), whereas a program's `emit` message (`op: "emit"`) is delivered to
  pages' `window.orzma.on(event, …)`. Same concept ("named event"), two `op`
...
- One handle = one `orzma://<handle>/` origin (isolated from other handles).
...
- **Ownership:** a `mount;<handle>` only takes effect in the pane whose
  `$ORZMA_TOKEN` registered that handle (a program mounts its own handles in its
  own pane).
...
### 5. The `orzma://` origin & asset serving

- URL shape: `orzma://<handle>/<path>`; empty path → `index.html`.
...
- `url`: remote `http(s)` loaded directly; **no** `orzma://` origin.

### 6. The `window.orzma` bridge (page side)
...
- **Availability:** present only on **bridged** views — `dir`/`inline` always;
  ...
  (`isOrzmaAvailable()` / `typeof window.orzma`).
- **API** (mirror `OrzmaApi` from `sdk/orzma-web`):
...
- **Binary round-trip rule:** a **top-level** `Uint8Array` round-trips (tagged
  ...
  foot-gun.
- **TS worked example** using `@orzma/web`.
...
Keep the existing `## Orzma Webview Protocol` section's short prose (lines
54–59) and append one link line, matching the `## Configuration` section's
pattern (`See [docs/configs.md](docs/configs.md) for ...`):

> See [docs/orzma_webview_protocol.md](docs/orzma_webview_protocol.md) for the
...
- `docs/orzma_webview_protocol.md` exists, is English, and follows the section
  structure above.
- Every control-socket message (both directions), the OSC grammar, the
  `orzma://` rules, and the `window.orzma` API are documented to field/byte
...
- The doc contains: one architecture diagram, one NDJSON worked example, one
  literal OSC byte example, one `window.orzma` TS snippet.
...
- OSC grammar & limits — `crates/orzma_tty_engine/src/osc/webview.rs`
...
- Control-socket NDJSON messages — `crates/orzma_webview/src/control_plane/protocol.rs`
...
- Handle minting, env injection, register validation/error codes, ownership —
  `crates/orzma_webview/src/control_plane.rs` (`mint_id` lowercase base32;
  `surface_env` injects `ORZMA_SOCK`/`ORZMA_TOKEN`; `build_view` error codes
  `invalid_root`/`unsafe_entry`/`html_too_large` and `validate_url_source`
  `invalid_url`/`unsupported_scheme`; `MAX_INLINE_HTML=4 MiB`; `OrzmaSource::is_bridged`;
...
- Listener transport, peer-UID auth, hello-first, NDJSON framing —
  `crates/orzma_webview/src/control_plane/listener.rs` (`peer_uid` via
...
- Host→program `call`/`event`/`urlChanged` wire shapes —
  `crates/orzma_webview/src/webview/render.rs` (`{"op":"call",handle,reqId,method,params}`,
  `{"op":"event",handle,event,payload}`, `urlChanged` fire-and-forget).
- Page bridge & byte tagging — `crates/orzma_webview/src/webview/render/orzma_bridge.js`
  (`orzma.call`→`{kind:"orzma.call",reqId,method,params}`;
  `orzma.emit`→`{kind:"orzma.emit",event,payload}`; replies on `orzma` channel,
  events on `orzma.event`; `{__u8: base64}` top-level only; `window.orzma` frozen).
- `orzma://` scheme & asset serving — `crates/webview_host/src/orzma_scheme.rs`
...
- Page-side API surface — `sdk/orzma-web/src/orzma.ts` (`OrzmaApi`,
  `isOrzmaAvailable`, `orzma`; `call`/`on`/`off`/`emit` signatures and the
