...
- Produces:
  ...
  pub(crate) struct BatchContext<'a> {
      ...
      pub(crate) forward_chords: &'a [ozma_webview::NormalizedChord],
  }
...
Run: `cargo test -p ozmux --lib action` (single filter — cargo test takes ONE testname; two positional filters is invalid)
