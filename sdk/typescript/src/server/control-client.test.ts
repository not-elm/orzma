...
function tmpSock(): string {
  return path.join(os.tmpdir(), `ozmux-control-test-${process.pid}-${Math.random().toString(36).slice(2)}.sock`);
}
...
describe('callControl', () => {
  ...
  it('sends a call frame and resolves with the result payload', async () => {
    ...
    const server = net.createServer((conn) => {
      conn.on('data', (chunk) => {
        ...
        conn.write(JSON.stringify({ kind: 'result', id: frame.id, payload: { new_pane_id: '7', new_activity_id: '9' } }) + '\n');
      });
    });
  ...
  it('rejects when the host returns an error frame', async () => {
    ...
    const server = net.createServer((conn) => {
      conn.on('data', (chunk) => {
        const frame = JSON.parse(chunk.toString('utf8').trim());
        conn.write(JSON.stringify({ kind: 'error', id: frame.id, code: 'pane_not_found', message: 'nope' }) + '\n');
      });
    });
    await new Promise<void>((r) => server.listen(sock, r));
    process.env.OZMUX_CONTROL_SOCK_PATH = sock;
