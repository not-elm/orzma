# Phase 1 — Step 2: `@ozmux/sdk/host` Runtime Core Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build the pure-logic core of the single Node host runtime as a new `@ozmux/sdk/host` package export — `defineApi`, the `{__u8}` binary codec, the plugin loader (namespace merge, first-wins + warn), and the RPC dispatcher — all dependency-injected and unit-tested, with NO sockets, NO process bootstrap, and zero impact on the running app.

**Architecture:** A new `sdk/typescript/src/host/` module. `defineApi` is a zero-cost identity helper preserving plugin API literal types. `plugin-loader` merges per-plugin default-exported API objects into one namespace map (globally-unique namespaces; earlier plugin wins on collision, later one warned). `dispatch` invokes `api[ns][method](...args)` and encodes a top-level binary result via the `{__u8: base64}` envelope. Everything is a pure/async function over plain values; the socket server, the `node` host entry, and the Rust spawn/wiring come in Step 3.
...
Step 1 ✅ (capability foundation, Rust). **Step 2 (this doc)** = host runtime pure core (TS). Step 3 = socket server + `node` host entry + reshape `ExtensionManagerPlugin` to spawn one host + scan plugin roots → `ViewRegistry` (with the `entry`/`id` validation carried from Step 1) + asset `{plugin,path}` protocol. Step 4 = host-API bridge (Proxy injection, `cef.emit`, Rust capability gate on `GrantedNamespaces`, return path). Step 5 = remove old machinery. Step 6 = memo plugin migration + `plugins/*` root + E2E.

Spec: `docs/superpowers/specs/2026-06-11-phase1-single-host-process-design.md` (§②, §③).

> **Conventions (.claude/rules/typescript.md):** JSDoc on every `export` whose meaning isn't obvious; comment taxonomy `// TODO:` / `// NOTE:` / `// biome-ignore <rule>: <reason>` only; named imports with `.ts` extensions (repo uses `allowImportingTsExtensions` + `verbatimModuleSyntax`); minimize export surface (these exports ARE the `@ozmux/sdk/host` public API, consumed by Step 3's host entry + the Rust-spawned process). A package-entry `index.ts` barrel is the established pattern (`src/server/index.ts`, `src/surface/index.ts`).
> **Run from `sdk/typescript/`:** tests `pnpm test` (vitest); types `pnpm check-types`. Lint from repo root: `pnpm lint` (biome over `sdk/**`). All must be clean before each commit.
...
| --- | --- | --- |
| `sdk/typescript/src/host/define-api.ts` | `defineApi` identity helper + `ApiNamespaceMap`/`ApiMethod` types | Create |
| `sdk/typescript/src/host/binary-codec.ts` | `{__u8}` base64 envelope encode/decode (boundary-tagged) | Create |
| `sdk/typescript/src/host/plugin-loader.ts` | `mergeApis` (first-wins + warn) + `loadPlugin` (injected importer) | Create |
| `sdk/typescript/src/host/dispatch.ts` | `dispatchHostCall(api, frame)` → result frame | Create |
| `sdk/typescript/src/host/index.ts` | Package-entry barrel for `@ozmux/sdk/host` | Create |
| `sdk/typescript/package.json` | Add the `./host` export | Modify |
| `*.test.ts` next to each module | vitest suites | Create |

---

## Task 1: `defineApi` + types, and the `./host` package export

**Files:**
- Create: `sdk/typescript/src/host/define-api.ts`
- Create: `sdk/typescript/src/host/define-api.test.ts`
- Create: `sdk/typescript/src/host/index.ts`
- Modify: `sdk/typescript/package.json`

- [ ] **Step 1: Write the failing test** — `sdk/typescript/src/host/define-api.test.ts`:
...
Run (from `sdk/typescript/`): `pnpm test define-api`
Expected: FAIL — cannot find module `./define-api.ts`.

- [ ] **Step 3: Implement** — `sdk/typescript/src/host/define-api.ts`:
...
- [ ] **Step 4: Create the package barrel** — `sdk/typescript/src/host/index.ts`:
...
- [ ] **Step 5: Add the `./host` export to `sdk/typescript/package.json`**

In the `"exports"` object, add this entry immediately after the `"./server"` block (before `"./cmd-shim"`):
...
Run (from `sdk/typescript/`): `pnpm test define-api` → PASS (2 tests).
Run: `pnpm check-types` → no errors.

- [ ] **Step 7: Commit**

```bash
git add sdk/typescript/src/host/define-api.ts sdk/typescript/src/host/define-api.test.ts sdk/typescript/src/host/index.ts sdk/typescript/package.json
git commit -m "feat(sdk/host): add defineApi identity helper and ./host export"
```
...
**Files:**
- Create: `sdk/typescript/src/host/binary-codec.ts`
- Create: `sdk/typescript/src/host/binary-codec.test.ts`
- Modify: `sdk/typescript/src/host/index.ts` (re-export)

- [ ] **Step 1: Write the failing test** — `sdk/typescript/src/host/binary-codec.test.ts`:
...
- [ ] **Step 3: Implement** — `sdk/typescript/src/host/binary-codec.ts`:
...
- [ ] **Step 4: Re-export from the barrel** — append to `sdk/typescript/src/host/index.ts`:
...
```bash
git add sdk/typescript/src/host/binary-codec.ts sdk/typescript/src/host/binary-codec.test.ts sdk/typescript/src/host/index.ts
git commit -m "feat(sdk/host): add boundary-tagged {__u8} binary codec"
```

---

## Task 3: plugin loader (merge first-wins + warn, injected importer)

**Files:**
- Create: `sdk/typescript/src/host/plugin-loader.ts`
- Create: `sdk/typescript/src/host/plugin-loader.test.ts`
- Modify: `sdk/typescript/src/host/index.ts` (re-export)

- [ ] **Step 1: Write the failing test** — `sdk/typescript/src/host/plugin-loader.test.ts`:
...
describe('loadPlugin', () => {
  ...
  it('rejects when the module has no object default export', async () => {
    ...
    await expect(loadPlugin('bad', '/abs/bad/api.ts', importer)).rejects.toThrow(/default-export an object/);
  });
...
Run: `pnpm test plugin-loader` → FAIL (module not found).

- [ ] **Step 3: Implement** — `sdk/typescript/src/host/plugin-loader.ts`:
...
Run: `pnpm test plugin-loader` → PASS (5 tests). `pnpm check-types` → clean.

- [ ] **Step 6: Commit**

```bash
git add sdk/typescript/src/host/plugin-loader.ts sdk/typescript/src/host/plugin-loader.test.ts sdk/typescript/src/host/index.ts
git commit -m "feat(sdk/host): add plugin loader with first-wins namespace merge"
```
...
**Files:**
- Create: `sdk/typescript/src/host/dispatch.ts`
- Create: `sdk/typescript/src/host/dispatch.test.ts`
- Modify: `sdk/typescript/src/host/index.ts` (re-export)

- [ ] **Step 1: Write the failing test** — `sdk/typescript/src/host/dispatch.test.ts`:
...
- [ ] **Step 3: Implement** — `sdk/typescript/src/host/dispatch.ts`:
...
Run (from `sdk/typescript/`): `pnpm test host` → PASS (all host tests: 2 + 6 + 5 + 5 = 18). `pnpm check-types` → clean.
Run (from repo root): `pnpm lint` → biome clean over the new files (fix with `pnpm lint:fix` if needed, then re-run).

- [ ] **Step 6: Commit**

```bash
git add sdk/typescript/src/host/dispatch.ts sdk/typescript/src/host/dispatch.test.ts sdk/typescript/src/host/index.ts
git commit -m "feat(sdk/host): add RPC dispatcher invoking api[ns][method]"
```
...
- `pnpm --filter @ozmux/sdk test` (or `pnpm test` in `sdk/typescript/`) green for all `src/host/*.test.ts` (18 tests).
- `pnpm check-types` clean; `pnpm lint` (biome) clean over `src/host/**`.
- No existing SDK test affected; no runtime/Rust behavior changed (pure new module, not yet imported anywhere).
- `@ozmux/sdk/host` exports `defineApi`, `ApiNamespaceMap`/`ApiMethod`, the binary codec, `mergeApis`/`loadPlugin`, and `dispatchHostCall`.

After this step lands, Step 3 adds the socket server (`bindHostRpcServer`), the `node` host entry (env-wired plugin discovery using `loadPlugin` + native `import()`), reshapes `ExtensionManagerPlugin` to spawn exactly one host, scans plugin roots → `ViewRegistry` (with `entry` path-traversal + `id` validation carried from Step 1), and extends the asset `Request` to `{plugin, path}`.
...
2. **User-plugins-first ordering (intentional reversal):** the array passed to `mergeApis` must be `[...userPlugins, ...bundledPlugins]` so user plugins win first-wins collisions. This reverses the current `extension_manager.rs` bundled-first order — mark the scan site with a `// NOTE:`.
