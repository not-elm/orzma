...
| - | - | - | - | - |
| RIS | `ESC c` | `1B 63` | 端末を初期状態へ戻す。 | p.90、動作定義は p.331 |
| DECKPAM | `ESC =` | `1B 3D` | 補助キーパッドをアプリケーションモードにする。 | p.94 |
| DECKPNM | `ESC >` | `1B 3E` | 補助キーパッドを数値モードに戻す。 | p.95 |

RIS は terminfo の `rs1=\Ec` に対応し、`reset(1)` や `tput reset` の実体である。未実装のままだと、画面が壊れたときの利用者側の復旧手段が無反応になる。vt510 p.331 は RIS の動作として、画面とページメモリの消去、カーソルのホーム復帰、SGR の標準表示への復帰、既定の文字集合の選択を挙げている。

DECKPAM / DECKPNM は terminfo の `smkx=\E[?1h\E=` と `rmkx=\E[?1l\E>` に対応する。さらに `ESC >` は初期化文字列 `is2` と `rs2` にも含まれるため、送出頻度は RIS より高い。

alacritty は 3 つとも実装している。
...
| 制御関数 | 必要なもの |
| - | - |
| RIS | `DeviceState::reset()`。両スクリーンの消去、スクロールバックの破棄、マージン・DECOM・タブ停止位置・文字集合・SGR・カーソル位置の初期化。 |
