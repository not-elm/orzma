...
### 最小権限の粒度
capability は **namespace 単位**。`capabilities = ["fs"]` は `window.fs.*` の **全メソッド**を許可する(`read` だけではない)。プラグイン作者は 1 namespace に危険メソッドと安全メソッドを混在させないこと。method 粒度の grant(例 `"fs.read"`)は将来課題(§5)。
...
| ② | `plugins/<name>/{api.ts, ozmux.toml, assets}`。`export default {...}`(任意で `defineApi(...)`)、namespace はグローバル一意・衝突は先勝ち+警告。capability は Rust 所有・namespace 粒度 |
...
| ④ | 制御プレーン / Handler / コマンド / bootstrap を全廃。OSC mount・`ViewRegistry`・`ozmux-ext://` scheme・`EndpointRegistry`・`JsEmitEvent`/`HostEmitEvent` は転用。`plugins/*` を追加ルートとして導入(`extensions/*→plugins/*` 改称は後続クリーンアップ) |
...
- **host runtime はアプリ同梱**(ユーザー提供ではない)。`@ozmux/sdk/host` の新エントリで、`<repo>/plugins/*` と `~/.config/ozmux/plugins/*` の `api.ts` を **dynamic import** して API オブジェクトを集約し、アセット配信と RPC ディスパッチを担う。
- **spawn 時の env:** 単一 RPC ソケットパス・アセットソケットパス・plugins ディレクトリパス。ランタイムルート(ソケット置き場)は **1 つだけ**(0700 perms)。現状の per-PID / per-extension ディレクトリツリーは廃止。
- **readiness:** host が全プラグインのロード後に `.ready` を返す。ロード失敗(後述の type-stripping 制約違反を含む)は名前付きで報告 → Rust 側でログ。Rust はタイムアウト付きで待機。
...
プラグインは npm パッケージではない(SDK `bootstrap()` 依存が廃止されるため)。1 プラグイン = 3 要素:

```
~/.config/ozmux/plugins/<name>/
├── api.ts          # export default { fs: { read(path) {...} } }
├── ozmux.toml      # views と必要 capability の宣言(Rust が読む信頼データ)
└── <assets>        # index.html 等。ozmux-ext://<plugin>/<entry> で配信
```
...
// 型推論を効かせたい場合(任意・@ozmux/sdk を解決できるプラグインのみ)
import { defineApi } from "@ozmux/sdk/host";
...
- **`defineApi` は任意の型推論用シュガー**。SDK が `export function defineApi<const T extends ApiNamespaceMap>(api: T): T { return api }` を提供し、将来 `typeof` ベースの `window.<ns>` 型 codegen の布石にする。**ただし `@ozmux/sdk/host` を import 解決できるのは workspace 内プラグインのみ**(下記モジュール解決の論点)。スタンドアロンなユーザープラグインは import を省いてプレーン default export を書く。
- **default export のトップレベルキーが namespace**(memo の `fs` がそのまま namespace)。1 プラグインが複数 namespace を提供可。
- **namespace はグローバルに一意**。複数プラグインで衝突したら、ロード順(プラグインディレクトリ名のソート順)で **先勝ち**、後発の衝突 namespace は **スキップ+警告ログ**(fail-soft)。

> **オープン論点(モジュール解決):** スタンドアロンな `~/.config/ozmux/plugins/<name>/api.ts` は npm パッケージではないため `@ozmux/sdk/host` を自動解決できない。かつ Node は `node_modules` 配下の TS を stripping しない。Phase 1 の既定は「ユーザープラグインは SDK を import しないプレーン default export」。SDK を使いたいプラグイン向けの解決策(import map / `NODE_PATH` / workspace 限定 / `tsx`)は §5 で決める。
...
- view が要求した capability に対応する namespace をどのプラグインも提供しない場合 → ロード警告。

**スキャン対象:** 同梱(`<repo>/plugins/`)+ ユーザー(`~/.config/ozmux/plugins/`)の両方。名前重複は **ユーザー優先**(上書き可能)。
...
**型付け**
- ランタイムは Proxy(動的)。Phase 1 は **プラグインが `.d.ts` で `Window` を augment** する軽量方式で足りる(完全な codegen は将来課題。手書き augment は `api.ts` 実体と乖離し得る点に留意)。
...
- `EndpointRegistry` ——**そのまま残し、全プラグイン名を「単一 host のアセットソケット」1 つへ向ける**(name→endpoint マップは不変、scheme handler は無改修)。
- `ozmux-ext://` scheme + アセット配信。bevy_cef 連携、`extension_render` の surface 描画。

> **アセットプロトコルの変更(必須):** `scheme.rs` は `ozmux-ext://<name>/<path>` を `<name>` で dispatch するが、現行の `fetch(&endpoints, path)` / `protocol::Request { path }` は **`<name>` を落として `path` だけ**を host へ渡す(`scheme.rs:75`, `protocol.rs:13`)。全名が同一ソケットを指す単一 host 構成では、host が**どのプラグインか判別できない**。アセットリクエストを **`{ plugin, path }`**(または `plugin/path` プレフィックス)へ拡張する。Node 側 `serveAssets` と protocol fixture も更新。

**移行(example / test fixture)**
- 現 `@memo` を **新モデルの同梱サンプルプラグイン**に作り替え:`plugins/memo/{api.ts(例: `fs` namespace), index.html(`window.fs.read` を呼ぶ), ozmux.toml(view `memo.main`, capabilities=["fs"])}`。正典の例 + 統合テスト fixture とする。
- **`plugins/*` を追加ルートとして先に導入**(host-API モデルに必要なのはこれだけ)。`extensions/* → plugins/*` への全面改称(pnpm workspace glob・docs・サンプル)は**後続のクリーンアップ**として分離し、Phase 1 のクリティカルパスから外す。

**SDK 変更**
- `@ozmux/sdk`:`./server`(bootstrap/control/handlers)・`./cmd-shim` を **削除**。
- **`@ozmux/sdk/host` を新設** ——同梱 host ランタイム(plugin loader=api.ts の dynamic import、アセットサーバ、単一ソケット上の RPC ディスパッチ、`defineApi`)。`node` が実行するのはこれ。
...
- **アセットルーティング:** `{plugin, path}` 拡張リクエストが正しいプラグインのアセットへ解決されること。
- **host ライフサイクル:** `host_unavailable` 時のグレースフル reject。

**host ランタイム(vitest, `@ozmux/sdk/host`)**
- plugin loader が複数 `api.ts`(プレーン default export と `defineApi` 形の両方)の namespace を集約、重複 namespace を先勝ちで解決。
...
**統合(E2E)**
- 同梱 `memo` プラグインで host プロセスを起動 → OSC mount → webview が `window.fs.read` を呼び期待バイト列を取得 → 未許可 namespace は reject。既存の `bevy_terminal/tests` / `extension_render` ハーネスを再利用。

**既知の注意:** 既存の IME テスト failure + 並列 teardown SIGSEGV があるため、グリーン確認は `--test-threads=1` + 該当 `--skip` を用いる。

## 5. オープンな論点 / 将来課題

- **SDK モジュール解決(要決定):** スタンドアロンなユーザープラグインが `@ozmux/sdk/host`(`defineApi` 等)を使う手段 — import map / `NODE_PATH` / workspace 限定 / `tsx` 同梱ローダのいずれか。
