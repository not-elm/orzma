...
**Tech Stack:** Rust 2024, Bevy 0.18.1 ECS (`EntityEvent` + observers, `run_if(in_state)`, `MessageReader`), `alacritty_terminal`, tmux control-mode client (`ozmux_tmux`).
...
- **Modify** `src/input/bindings.rs` — delete `TerminalInputBindings` + `ReservedChord` (keyboard policy only); KEEP `OzmaMouseConfig`, `FineModifier` (mouse policy).
...
**Interfaces:**
- Consumes: `LeaderPhase`, `Shortcuts`, `step_leader`, `LeaderStep`, `is_modifier_key` (from `crate::input::shortcuts`); `Modifiers`, `ShortcutAction` (from `ozmux_configs::shortcuts`); `ResolvedCopyModeKeys`, `CopyModeAction` (from `crate::action::vi`); `bevy::input::keyboard::{Key, KeyCode, KeyboardInput}`; `ozma_webview::ForwardKeys` inner chord type for `forward_chords`.
- Produces:
  ```rust
  ...
  pub(crate) struct BatchContext<'a> {
      ...
      pub(crate) forward_chords: &'a [/* ForwardKeys chord */],
  }
...
repeat_window_withholds_matching_key         // Repeat + repeat-marked 'h' -> [] (no Type)
...
release_webview_chord_swallowed_no_webview   // Ctrl+Shift+Escape, webview_focused:false -> [] (no Type)
...
Run: `cargo test -p ozmux --lib input::resolve`
...
4. Else (not in copy mode): if the key matches `is_release_webview_focus` → nothing (reserved-chord withhold parity); if `ctx.mods.meta` → nothing (meta-drop); else → `Type { logical: ev.logical_key.clone(), key_code: ev.key_code }`.
...
Make `is_modifier_key` handling implicit via `step_leader` (it already returns `Passthrough` for bare modifiers); the withhold "skip bare modifier for the suppression slot" is naturally handled because a bare modifier resolves to `Passthrough` → `match_gui_action` None → (not copy) → meta/`Type` path; a bare modifier's `logical_key` maps to `None` in `bevy_key_to_terminal_key`, so it types nothing. Verify the ported tests cover `pending_skips_bare_modifier_then_second_key`.
...
```bash
cargo clippy -p ozmux --all-targets && cargo fmt
...
Run: `cargo test -p ozmux --lib action::tmux::detach_session`
...
```
`request_detach` stays in `src/session/tmux.rs` (already `pub(crate)`); this observer calls it. Add `DetachSessionPlugin` to `TmuxActionPlugin`'s tuple in `src/action/tmux.rs`.
...
Run: `cargo test -p ozmux --lib input::tmux::forward`
...
```
Register `.add_observer(on_forward_pane_keys)` in `ForwardPlugin::build`. NOTE: preserve the current ordering — snap/flush BEFORE the send, exactly as `input.rs:411-431`.
...
Run: `cargo test -p ozmux --lib action::tmux::paste action::terminal::paste`
...
fn on_paste_tmux(
    ...
    mut handles: Query<&mut TerminalHandle>,
    mut client: Option<Single<&mut TmuxClient>>,
    panes: Query<&TmuxPane, With<TmuxPane>>,
) {
...
match effect {
    ...
    KeyEffect::Type { logical, .. } =>
        if let Some(key) = bevy_key_to_terminal_key(&logical) {
...
Run: `cargo test -p ozmux --lib input::tmux::input`
...
Run: `cargo test -p ozmux --lib input::tmux && cargo build -p ozmux`
Expected: PASS + clean build. Then attach a `tmux -CC` session (`cargo run`) and confirm: typing reaches the pane, `<Leader>` pane/window ops work, paste, detach (Ctrl+Shift+D), copy-mode entry + vi nav, webview forward-keys, and that plain keys still batch (one `SendPaneKeys` per frame — check `RUST_LOG=ozmux_tmux=trace`).
