//! Minimal remote-URL webview demo. Run inside an orzma pane:
//! `cargo run -p ratatui-orzma --example ratatui_remote_url`.
//!
//! Mounts a display-only remote page (no `window.orzma` bridge — `Webview::url`
...
use ratatui_orzma::{Orzma, OrzmaBackend, Webview, WebviewWidget};
use std::io::stdout;
use std::time::Duration;

fn main() -> Result<(), Box<dyn std::error::Error>> {
    let orzma = Orzma::connect()?;
    let view = orzma.register(Webview::url("https://github.com/not-elm?tab=repositories"))?;
