...
- [ ] CI に macOS ジョブが無いので、macOS 専用の `unsafe` な FFI（`proc_pidinfo` / `proc_listchildpids`）が CI でコンパイルもテストもされない。macOS ジョブを足す。
- [ ] OS から得たディレクトリは `is_dir()` しか確かめないので、存在しても実行（検索）権限の無いディレクトリだと spawn の chdir が失敗し、分割そのものが `SpawnFailed` になる。入れるかどうかを確かめてから採用するか、失敗時にホームディレクトリで起動し直す。
