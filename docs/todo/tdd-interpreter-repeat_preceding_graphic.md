...
`REP`（`CSI Pn b`）を受け持つ `Executor::repeat_preceding_graphic`（`crates/orzma_vt/src/interpreter.rs:582`）のテストケース一覧。
...
```
PG1 — crates/orzma_vt/src/device.rs:137-139, DeviceState::preceding_graphic
      "The graphic character `REP` repeats: the last character one or two columns wide that [`Self::print`] mapped, or `None` when none has been printed since power-up or the last `RIS`."
      justifies: 繰り返すのは変換後のグリフで、何も表示していなければ記憶はない
PG2 — crates/orzma_vt/src/device.rs:141-142, DeviceState::preceding_graphic
...
DP  — crates/orzma_vt/src/device.rs:116-119, DeviceState::print
...
RS  — crates/orzma_vt/src/device.rs:192-193, DeviceState::reset
      "The preceding graphic character is cleared, so a `REP` that follows prints nothing."
      justifies: RIS の後の REP は何も出さない
RP1 — crates/orzma_vt/src/interpreter.rs:571-573, Executor::repeat_preceding_graphic
...
RP2 — crates/orzma_vt/src/interpreter.rs:575, Executor::repeat_preceding_graphic
      "A pending single shift stays pending."
      justifies: 保留中の SS2 は繰り返しで使われない
RP3 — crates/orzma_vt/src/interpreter.rs:575-577, Executor::repeat_preceding_graphic
      "Nothing is printed when no graphic character has been printed since power-up or the last `RIS`, and the repetitions stop at the first glyph a row refuses."
      justifies: 記憶がなければ何も出さず、カーソルも動かない
RC  — crates/orzma_vt/src/interpreter.rs:896, repeat_count
      "A repeat count parameter, where an omitted or zero value means one."
      justifies: （C5 と矛盾するため、ケースには使っていない）
SPG1 — crates/orzma_vt/src/screen.rs:214-216, Screen::print_graphic
...
SPG2 — crates/orzma_vt/src/screen.rs:210-211, Screen::print_graphic
