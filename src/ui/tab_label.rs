...
//! `Cwd` (home-abbreviated, front-truncated), blank until the cwd is known;
...
mod tests {
    ...
    fn terminal_under_home_abbreviates() {
        ...
        let out = tab_label(Some(&cwd), Some(Path::new("/home/u")), MAX);
        assert_eq!(out, "~/proj");
    }
    ...
    fn no_home_keeps_absolute() {
        let cwd = Cwd(PathBuf::from("/home/u/proj"));
        let out = tab_label(Some(&cwd), None, MAX);
    ...
    fn tiny_max_chars_never_exceeds_budget() {
        ...
        let out0 = tab_label(Some(&cwd), Some(Path::new("/home/u")), 0);
        assert_eq!(out0.chars().count(), 0, "got {out0:?}");
        let out1 = tab_label(Some(&cwd), Some(Path::new("/home/u")), 1);
    ...
    fn trailing_slash_does_not_leak_or_overflow() {
        ...
        let tiny = tab_label(Some(&cwd), None, 1);
