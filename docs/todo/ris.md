...
| 4 | `interpreter.rs` | `Executor::esc_dispatch` の `(b'c', [])` アーム＋private な handler | #3 を呼び、その返り値を既存の `Executor::stage` に渡すだけ（`DamageSpan::Full` 固定ではなくなった）。placement の破棄に専用の呼び出しは要らない。[placement-ownership-design.md](../superpowers/specs/2026-08-26-placement-ownership-design.md) の「帰結 — `Screen::reset` は placement について何も返さない」節が示すとおり、`#2`（`Screen::reset`）が呼ぶ `Grid::reset` は行 id を振り直すので RIS 前のどの anchor も解決不能になり、あとはチャンク末尾の `DeviceState::evict_lost_anchors`（両スクリーン sweep）が通常どおり回収する。handler 側が全消し専用の API や signal 送出を持つ必要はない。 |
