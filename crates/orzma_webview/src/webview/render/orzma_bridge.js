...
// arrive on the "orzma" channel keyed by reqId; push events on "orzma.event".
// Page->host one-way events use orzma.emit -> cef.emit({kind:'orzma.emit'}):
...
// window.orzma is frozen so a page cannot shadow it.
(function () {
  var cef = window.cef;
  var nextId = 0;
  ...
  cef.listen('orzma.event', function (raw) {
    var frame = typeof raw === 'string' ? JSON.parse(raw) : raw;
  ...
  var api = {
    call: function (method, params) {
      ...
      return new Promise(function (resolve, reject) {
        ...
        try {
          cef.emit({ kind: 'orzma.call', reqId: reqId, method: method, params: encoded });
        } catch (e) {
    ...
    emit: function (event, payload) {
      cef.emit({ kind: 'orzma.emit', event: event, payload: encodeParam(payload) });
    },
  };

  Object.defineProperty(window, 'orzma', { value: Object.freeze(api), configurable: false, writable: false });

  // NOTE: length > 1 means a page-specific handler is also registered (e.g. orzmd's);
