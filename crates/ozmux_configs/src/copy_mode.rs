//! `[copy-mode]` table: shared copy-mode key bindings for both Default and
...
//! chords, matching how copy-mode keys are actually decided at runtime.
...
/// Named (non-character) keys accepted in `[copy-mode]` bindings.
#[derive(Clone, Copy, Debug, PartialEq, Eq, PartialOrd, Ord, Hash)]
pub enum CopyModeNamedKey {
    /// `Escape`.
    Escape,
    /// `Enter`.
    Enter,
...
/// The key part of a `[copy-mode]` binding: an exact logical character
/// (case-sensitive — `"V"` means Shift+v as typed) or a named key.
#[derive(Clone, Debug, PartialEq, Eq, PartialOrd, Ord, Hash)]
pub enum CopyModeBaseKey {
...
/// One `[copy-mode]` binding entry: an optional `Ctrl+` prefix plus a key.
///
/// # Invariants
///
...
#[derive(Clone, Debug, PartialEq, Eq, PartialOrd, Ord, Hash)]
pub struct CopyModeKey {
    /// `Ctrl` is part of the binding.
    pub ctrl: bool,
    /// The key to match.
    pub key: CopyModeBaseKey,
}

impl fmt::Display for CopyModeKey {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        if self.ctrl {
            write!(f, "Ctrl+")?;
        }
        match &self.key {
            CopyModeBaseKey::Char(c) => write!(f, "{c}"),
            CopyModeBaseKey::Named(n) => write!(f, "{}", n.token()),
        }
    }
}

/// Reason a copy-mode key string failed to parse.
#[derive(Debug, Clone, PartialEq, Eq, thiserror::Error)]
pub enum CopyModeKeyParseError {
    /// Empty key token.
    #[error("empty copy-mode key")]
    Empty,
    /// A modifier other than `Ctrl` (`Cmd`/`Alt`/`Shift`/aliases).
    #[error(
        "modifier {0:?} is not allowed in [copy-mode] (only Ctrl+); express Shift via the character case"
    )]
    ForbiddenModifier(String),
    /// More than one `+`-separated segment beyond `Ctrl+<key>`.
    #[error("too many tokens in copy-mode key {0:?} (expected [Ctrl+]<key>)")]
    TooManyTokens(String),
    /// A multi-character token that is not a known named key.
    #[error("unknown copy-mode key {0:?} (expected one character or a named key)")]
    UnknownKey(String),
...
/// Parses one `[copy-mode]` key string (`"V"`, `"$"`, `"Ctrl+F"`, `"Escape"`).
pub fn parse_copy_mode_key(s: &str) -> Result<CopyModeKey, CopyModeKeyParseError> {
    if s.is_empty() {
    ...
    let (ctrl, key_token) = match parts.as_slice() {
        ...
        [modifier, _] => {
            return Err(CopyModeKeyParseError::ForbiddenModifier(
                (*modifier).to_string(),
            ));
        }
        _ => return Err(CopyModeKeyParseError::TooManyTokens(s.to_string())),
    };
    if key_token.is_empty() {
        return Err(CopyModeKeyParseError::Empty);
    }
    if let Some(named) = CopyModeNamedKey::from_token(key_token) {
        return Ok(CopyModeKey {
            ctrl,
            key: CopyModeBaseKey::Named(named),
        });
    }
    let mut chars = key_token.chars();
    let (Some(c), None) = (chars.next(), chars.next()) else {
        return Err(CopyModeKeyParseError::UnknownKey(key_token.to_string()));
    };
    if ctrl {
        if !c.is_ascii_alphanumeric() {
            return Err(CopyModeKeyParseError::CtrlNonAlphanumeric(
                key_token.to_string(),
            ));
        }
        return Ok(CopyModeKey {
    ...
    Ok(CopyModeKey {
        ctrl: false,
        key: CopyModeBaseKey::Char(c.to_string()),
    })
}
...
/// One copy-mode action, in config-crate (engine-free) vocabulary.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
...
/// `OzmuxConfigsError::DuplicateCopyModeKeys`.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct DuplicateCopyModeKey {
...
macro_rules! copy_mode_fields {
    ($($(#[$doc:meta])* $field:ident => $label:literal, $action:expr, [$($default:literal),*];)+) => {
        /// User-facing `[copy-mode]` configuration: one flat table of copy-mode
        ...
        impl CopyModeConfig {
            ...
            /// The single source of truth for the `[copy-mode]` schema.
            pub fn bindings_iter(
                &self,
            ) -> impl Iterator<Item = (&'static str, &Vec<CopyModeKey>, CopyModeAction)> + '_ {
                [
...
copy_mode_fields! {
    ...
    cursor_left => "cursor-left", CopyModeAction::Motion(CopyMotion::Left), ["h", "ArrowLeft"];
    /// Move the cursor one cell down.
    cursor_down => "cursor-down", CopyModeAction::Motion(CopyMotion::Down), ["j", "ArrowDown"];
    /// Move the cursor one cell up.
    cursor_up => "cursor-up", CopyModeAction::Motion(CopyMotion::Up), ["k", "ArrowUp"];
    /// Move the cursor one cell right.
    cursor_right => "cursor-right", CopyModeAction::Motion(CopyMotion::Right), ["l", "ArrowRight"];
    /// Jump to column 0.
    line_start => "line-start", CopyModeAction::Motion(CopyMotion::LineStart), ["0"];
    /// Jump to the last column.
    line_end => "line-end", CopyModeAction::Motion(CopyMotion::LineEnd), ["$"];
    /// Jump to the first non-blank column.
    line_first_char => "line-first-char", CopyModeAction::Motion(CopyMotion::LineFirstChar), ["^"];
    /// Jump to the next word start.
    next_word => "next-word", CopyModeAction::Motion(CopyMotion::NextWord), ["w"];
    /// Jump to the previous word start.
    previous_word => "previous-word", CopyModeAction::Motion(CopyMotion::PreviousWord), ["b"];
    /// Jump to the next word end.
    next_word_end => "next-word-end", CopyModeAction::Motion(CopyMotion::NextWordEnd), ["e"];
    /// Jump to the next space-delimited word start.
    next_space => "next-space", CopyModeAction::Motion(CopyMotion::NextSpace), ["W"];
    /// Jump to the previous space-delimited word start.
    previous_space => "previous-space", CopyModeAction::Motion(CopyMotion::PreviousSpace), ["B"];
    /// Jump to the next space-delimited word end.
    next_space_end => "next-space-end", CopyModeAction::Motion(CopyMotion::NextSpaceEnd), ["E"];
    /// Jump to the top visible line.
    screen_top => "screen-top", CopyModeAction::Motion(CopyMotion::ScreenTop), ["H"];
    /// Jump to the middle visible line.
    screen_middle => "screen-middle", CopyModeAction::Motion(CopyMotion::ScreenMiddle), ["M"];
    /// Jump to the bottom visible line.
    screen_bottom => "screen-bottom", CopyModeAction::Motion(CopyMotion::ScreenBottom), ["L"];
    /// Jump to the previous paragraph boundary.
    previous_paragraph => "previous-paragraph", CopyModeAction::Motion(CopyMotion::PreviousParagraph), ["{"];
    /// Jump to the next paragraph boundary.
    next_paragraph => "next-paragraph", CopyModeAction::Motion(CopyMotion::NextParagraph), ["}"];
    /// Jump to the matching bracket.
    matching_bracket => "matching-bracket", CopyModeAction::Motion(CopyMotion::MatchingBracket), ["%"];
    /// Scroll to the oldest history line.
    history_top => "history-top", CopyModeAction::Scroll(CopyScroll::HistoryTop), ["g"];
    /// Scroll to the live tail.
    history_bottom => "history-bottom", CopyModeAction::Scroll(CopyScroll::HistoryBottom), ["G"];
    /// Scroll one page up.
    page_up => "page-up", CopyModeAction::Scroll(CopyScroll::PageUp), ["Ctrl+B"];
    /// Scroll one page down.
    page_down => "page-down", CopyModeAction::Scroll(CopyScroll::PageDown), ["Ctrl+F"];
    /// Scroll half a page up.
    half_page_up => "half-page-up", CopyModeAction::Scroll(CopyScroll::HalfPageUp), ["Ctrl+U"];
    /// Scroll half a page down.
    half_page_down => "half-page-down", CopyModeAction::Scroll(CopyScroll::HalfPageDown), ["Ctrl+D"];
    /// Scroll one line up.
    scroll_up => "scroll-up", CopyModeAction::Scroll(CopyScroll::ScrollUp), ["Ctrl+Y"];
    ...
    toggle_rect_selection => "toggle-rect-selection", CopyModeAction::Selection(CopySelection::Rect), ["Ctrl+V"];
    /// Copy the selection to the clipboard and leave copy mode.
    yank => "yank", CopyModeAction::Yank, ["y", "Enter"];
    /// Leave copy mode.
    exit => "exit", CopyModeAction::Exit, ["q", "Escape", "Ctrl+C"];
    /// Open the search-down prompt (tmux mode only for now).
    search_forward => "search-forward", CopyModeAction::Prompt(CopyPromptDir::SearchForward), ["/"];
    /// Open the search-up prompt (tmux mode only for now).
    search_backward => "search-backward", CopyModeAction::Prompt(CopyPromptDir::SearchBackward), ["?"];
    /// Repeat the previous search (tmux mode only for now).
    search_next => "search-next", CopyModeAction::SearchStep(CopySearchStep::Next), ["n"];
    /// Repeat the previous search, reversed (tmux mode only for now).
    search_previous => "search-previous", CopyModeAction::SearchStep(CopySearchStep::Previous), ["N"];
    /// Open the jump-to-char-forward prompt (tmux mode only for now).
    jump_forward => "jump-forward", CopyModeAction::Prompt(CopyPromptDir::JumpForward), ["f"];
    /// Open the jump-to-char-backward prompt (tmux mode only for now).
    jump_backward => "jump-backward", CopyModeAction::Prompt(CopyPromptDir::JumpBackward), ["F"];
    /// Open the jump-till-char-forward prompt (tmux mode only for now).
    jump_to_forward => "jump-to-forward", CopyModeAction::Prompt(CopyPromptDir::JumpToForward), ["t"];
    /// Open the jump-till-char-backward prompt (tmux mode only for now).
    jump_to_backward => "jump-to-backward", CopyModeAction::Prompt(CopyPromptDir::JumpToBackward), ["T"];
}

impl CopyModeConfig {
    /// Detects keys bound to more than one action. Deterministic order via
    /// `BTreeMap`.
    pub(crate) fn validate_no_duplicate_keys(&self) -> Result<(), Vec<DuplicateCopyModeKey>> {
        let mut by_key: BTreeMap<CopyModeKey, Vec<&'static str>> = BTreeMap::new();
        for (label, keys, _action) in self.bindings_iter() {
            for key in keys {
                by_key.entry(key.clone()).or_default().push(label);
            }
        }
        let dupes: Vec<DuplicateCopyModeKey> = by_key
            .into_iter()
            .filter(|(_, labels)| labels.len() >= 2)
            .map(|(key, actions)| DuplicateCopyModeKey { key, actions })
            .collect();
...
fn deser_keys<'de, D>(d: D) -> Result<Vec<CopyModeKey>, D::Error>
where
    D: serde::Deserializer<'de>,
{
    ...
    raw.iter()
        .map(|s| parse_copy_mode_key(s).map_err(DeError::custom))
        .collect()
}

fn ser_keys<S>(keys: &[CopyModeKey], ser: S) -> Result<S::Ok, S::Error>
where
    S: serde::Serializer,
{
...
fn parse_default_key(s: &str) -> CopyModeKey {
    parse_copy_mode_key(s).unwrap_or_else(|e| panic!("invalid default copy-mode key {s:?}: {e}"))
}
...
mod tests {
    ...
    fn bindings_iter_count_is_pinned_to_field_count() {
        // NOTE: drift guard — a CopyModeConfig field without its
        // bindings_iter() entry silently unbinds the action.
        assert_eq!(CopyModeConfig::default().bindings_iter().count(), 40);
    }
    ...
    fn parse_ctrl_lowercases_and_limits_domain() {
        assert_eq!(parse_copy_mode_key("Ctrl+F").unwrap(), ctrl("f"));
        assert_eq!(parse_copy_mode_key("ctrl+v").unwrap(), ctrl("v"));
        assert!(matches!(
            parse_copy_mode_key("Ctrl+$"),
            Err(CopyModeKeyParseError::CtrlNonAlphanumeric(_))
        ));
    }
    ...
    fn parse_rejects_forbidden_modifiers_and_garbage() {
        assert!(matches!(
            parse_copy_mode_key("Cmd+x"),
            Err(CopyModeKeyParseError::ForbiddenModifier(_))
        ));
        assert!(matches!(
            parse_copy_mode_key("Shift+v"),
            Err(CopyModeKeyParseError::ForbiddenModifier(_))
        ));
        assert!(matches!(
            parse_copy_mode_key("Alt+j"),
            Err(CopyModeKeyParseError::ForbiddenModifier(_))
        ));
        assert!(matches!(
            parse_copy_mode_key("ab"),
            Err(CopyModeKeyParseError::UnknownKey(_))
        ));
        assert!(matches!(
            parse_copy_mode_key(""),
            Err(CopyModeKeyParseError::Empty)
        ));
        assert!(matches!(
            parse_copy_mode_key("Ctrl+Shift+F"),
            Err(CopyModeKeyParseError::TooManyTokens(_))
        ));
    }

    #[test]
    fn toml_accepts_string_array_and_unbind() {
        let cfg: CopyModeConfig =
        ...
        assert_eq!(cfg.cursor_left, CopyModeConfig::default().cursor_left);
    }

    #[test]
    fn toml_rejects_unknown_action() {
        assert!(toml::from_str::<CopyModeConfig>("goto-line = \":\"\n").is_err());
    }

    #[test]
    fn default_has_no_duplicate_keys() {
        assert!(
            CopyModeConfig::default()
    ...
    fn duplicate_key_across_actions_is_detected() {
        let cfg: CopyModeConfig = toml::from_str("yank = \"x\"\nexit = [\"x\", \"q\"]\n").unwrap();
    ...
    fn defaults_match_spec_spot_checks() {
        ...
        assert_eq!(
            ...
            CopyModeKey {
                ctrl: false,
                key: CopyModeBaseKey::Named(CopyModeNamedKey::ArrowLeft),
            }
        );
        ...
        assert_eq!(
            ...
            vec![
                ...
                CopyModeKey {
                    ctrl: false,
                    key: CopyModeBaseKey::Named(CopyModeNamedKey::Enter)
                }
            ]
        );
    ...
    fn display_roundtrips_tokens() {
        ...
        assert_eq!(
            CopyModeKey {
                ctrl: false,
                key: CopyModeBaseKey::Named(CopyModeNamedKey::ArrowUp),
            }
            .to_string(),
            "ArrowUp"
        );
