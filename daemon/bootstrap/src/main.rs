...
fn main() -> Result<()> {
    ...
    // the second call is a no-op via `try_init()`. The TraceGuard bound
    // here must live for the whole main() so the tracing-chrome
    // FlushGuard (when active) flushes the writer thread on shutdown.
