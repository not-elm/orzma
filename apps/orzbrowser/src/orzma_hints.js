...
// `window.orzma` reads below.
(function () {
  var orzma = window.orzma;
  var ALPHABET = 'sadfjklewcmpgh';
  var OVERLAY_ID = '__orzmaHints';
  var state = null;
  ...
  function show() {
    ...
    var items = collect();
    if (items.length === 0) {
      orzma.call('hintResult', { kind: 'empty' });
      return;
    }
  ...
  function activate(t) {
    ...
    if (t.kind === 'input') {
      t.el.focus();
      orzma.call('hintResult', { kind: 'focusedInput' });
      return;
    }
    if (t.kind === 'link') {
      ...
      if (/^https?:\/\//i.test(href)) {
        orzma.call('hintResult', { kind: 'navigated', url: href });
        return;
      }
    }
    t.el.click();
    orzma.call('hintResult', { kind: t.kind === 'link' ? 'navigated' : 'clicked' });
  }
  ...
  orzma.on('hints:show', function () { show(); });
  orzma.on('hints:hide', function () { teardown(); });
  orzma.on('hints:key', function (payload) {
    if (!state) return;
