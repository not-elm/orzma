...
- `daemon/browser` (`ozmux_browser`) — headless Chromium service driven over CDP via `chromiumoxide`. Owns one shared Chromium process; each Browser Activity is one CDP page in the shared default `BrowserContext`. Frames are JPEG-screencast over `tokio::sync::watch` (one slot per activity, no delta ring). Cookie bootstrap uses the `decrypt-cookies` crate at process launch. A `ChromiumState` machine (`Stopped|Starting|Running|StoppingAfter`) with a 30 s idle-grace shutdown prevents thrashing on rapid open/close. `OZMUX_BROWSER_SITE_ISOLATION=1` — restore Chromium's default Site Isolation in the embedded Browser Activity. Off by default: cross-origin navigation would otherwise break the CDP session that `chromiumoxide 0.7` holds for the page, defeating viewport / input forwarding. Local single-user tool; the security trade-off is acceptable. `OZMUX_BROWSER_SKIP_COOKIE_IMPORT=1` — skip importing the host Chrome's cookies into the Browser Activity. On macOS the import reads the `Chrome Safe Storage` Keychain item via `decrypt-cookies`, which raises an authorization dialog for the ozmux binary on every launch; set this during local development to suppress it (the embedded browser then opens unauthenticated).
- `daemon/http_server` (`ozmux_http_server`) — axum router on `127.0.0.1:3200`. `AppState` aggregates `MultiplexerState` (`Arc<Mutex<MultiplexerService>>`), `TerminalService`, and `ExtensionRegistry`, each wired in via `FromRef`. Top-level REST nests are `/sessions`, `/windows`, `/configs`; panes and activities are nested under windows (e.g. `/windows/{wid}/panes/{pid}/activities/{aid}/...`). Per-activity endpoints include a terminal WebSocket (`/.../terminal/ws`, msgpack `WireMessage` frames), a browser WebSocket (`/.../browser/ws`, msgpack `BrowserServerMsg`/`BrowserClientMsg` frames, JPEG screencast), an extension handlers WebSocket (`/.../handlers/ws`), and an iframe passthrough (`/.../iframe/{*path}`). `serve` bootstraps one session/window/pane/activity and spawns its PTY before the listener binds. The embedded `index.html` lives at `src/handlers/index.html`, where `vite build` writes it.
