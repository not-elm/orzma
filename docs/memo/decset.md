...
| Ps | Set | Reset |
| - | - | - |
| 2026 | 同期更新を開く。以後の frame を止める | 同期更新を閉じる。閉じた瞬間の画面を frame にする（間引きの対象。下を参照） |

- パースは止めない。止めるのは frame の emit だけなので、DSR などの応答は遅れない。
  PTY が上げた signal も遅れないが、それを反映する frame は抑止される。下の「抑止の代償」を参照。
- `Vt::interpret` は同期更新を閉じたバイトで止まり、`consumed` を返す。呼び出し側（`orzma_tty`）は
  その瞬間の frame を取ってから残りを再投入する。
- **べき等**: 開いている間の `?2026h` と、開いていないときの `?2026l` は no-op。
- **タイムアウト**: 150 ms。締め切りは最初の `?2026h` に固定し、再 `?2026h` では延長しない。
  過ぎても `VtModes` のフラグには触らず、描画だけ再開する（DECRQM はアプリが set した値を返し続ける）。
- **閉じた瞬間の frame の間引き**: 直前の emit から 12 ms 未満なら、その close では frame を取らない。
  damage は残るので通常の窓で拾われるが、次の同期更新がその窓より先に開くと、次の close か
  タイムアウトまで待つ。タイムアウト後の描きかけ frame も間引きの基準に入るので、1 回の更新が
  150 ms を超える出力が続く間は、完成した画面が出ないことがある。1 つの frame が留まるのは
  最大で 12 + 150 ms。
- **抑止の代償**: control socket 経由の mount の signal は、出力の無い pane では最大 150 ms 遅れる。
  webview の unmount / evict の signal は即座に届くが、それを反映する frame は抑止されるので、
  アプリが close を送る前に止まると、webview が消えた跡に古いセルが見える。
- **抑止を貫通するもの**: resize 直後と pane を閉じるときの `flush_now` だけ。bootstrap の最初の frame も、
  ユーザーのスクロールと選択の描画も抑止に従う。
...
`ansi_mode_report` にある。

| 種別 | Ps | 応答 |
| - | - | - |
| DEC | 1, 7, 25, 66, 1004, 1007, 2004, 2026 | 状態に応じて 1 / 2 |
| DEC | 6 | 表示中の画面の状態に応じて 1 / 2 |
| DEC | 12 | `text_cursor.blink` に応じて 1 / 2。DECSCUSR が変えた点滅も反映する |
| DEC | 47, 1047, 1049 | 3 つとも「代替画面を表示中か」 |
| DEC | 1000, 1002, 1003 | 現在のレベルと一致すれば 1、しなければ 2 |
| DEC | 1006 | 現在のエンコーディングが SGR なら 1 |
| DEC | 3, 40, 95, 1005, 1034, 1048, その他 | 0 |
| ANSI | 4 | 状態に応じて 1 / 2 |
| ANSI | その他 | 0 |

- 3 / 40 / 95 への 0 は alacritty と同じ答えで、xterm は 2 を返す。受理しても状態を持たないモードには
  0 で答える、という規則で揃えている（1034 も同じ。1005 はどのマウスモードにも該当せず無視される。
  1048 は状態の無い操作）。
- 応答の `Ps` は受け取った整数をそのまま書く。表を引く値は `u16` に飽和するが、エコーは飽和させない。
- パラメータ省略は `Ps = 0` として `0;0` を返す。
- 2026 はタイムアウトで描画が再開したあとも、アプリが reset するまで 1 を返す。
...
`set_private_modes`の`_ => {}`はそのままでよい。DECRQM はそれらに 0（未認識）で答える。
