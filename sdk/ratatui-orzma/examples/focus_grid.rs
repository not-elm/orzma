...
//!
//! Run inside an orzma pane: `cargo run -p ratatui-orzma --example focus_grid`.
//!
...
//! webview. `WebviewWidget::focused` reports focus to the SDK; the `OrzmaBackend`
...
use ratatui_orzma::{KeyChord, Orzma, OrzmaBackend, Webview, WebviewHandle, WebviewWidget};
use std::io::Stdout;
use std::io::stdout;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let ne =
        orzma.register(Webview::inline(webview_html("NE webview", "#8be9fd")).forward_keys(nav))?;
    let sw =
        orzma.register(Webview::inline(webview_html("SW webview", "#bd93f9")).forward_keys(nav))?;
    ...
    let result = (|| -> Result<(), Box<dyn std::error::Error>> {
        ...
        run(&mut terminal, &mut orzma, &ne, &sw)
    })();
...
fn run(
    ...
    loop {
        terminal.draw(|f| draw(f, orzma, ne, sw, &focused, &last_key))?;
...
fn draw(
    ...
    // NOTE: Orzma::frame clears the collector, so call it once before rendering both
