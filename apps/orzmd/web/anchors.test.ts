...
describe('installHeadingAnchors', () => {
  it('injects a slug-id span into each heading', () => {
    ...
    expect(div.querySelector('h1 > span.orzmd-anchor')?.id).toBe('mounting');
    expect(div.querySelector('h2 > span.orzmd-anchor')?.id).toBe('foo-bar');
  });
  ...
  it('dedupes colliding slugs', () => {
    ...
    const ids = Array.from(div.querySelectorAll('span.orzmd-anchor')).map((s) => s.id);
  ...
  it('skips injecting a slug that collides with an existing positional id (Fix 2)', () => {
    ...
    expect(div.querySelector('h2:nth-of-type(2) > span.orzmd-anchor')).toBeNull();
