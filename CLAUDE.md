...
- `crates/bevy_orzma_tty_renderer` (`bevy_orzma_tty_renderer`) — GPU terminal renderer plus the grid schema `bevy_orzmux` populates via `TtyFrameSignal`. `TerminalRendererPlugin` wires the grid, material, and glyph sub-plugins (`TerminalGridPlugin`, `TerminalMaterialPlugin`, `TerminalGlyphPlugin`) and hyperlink-hover state; `schema` holds the cell/grid types both crates render against, split into `TerminalView` (viewport geometry, cursor, selection, placements) and `TerminalCells` (`orzma_vt` cells, palette, hyperlink table) so a cursor or selection change never re-uploads the GPU cell buffer.
...
`src/main.rs` plus: `action`, `cef_profile`, `configs`, `font`, `input`, `redraw`, `session`, `surface`, `system_set`, `ui`, `window_icon`, `window_title`.
