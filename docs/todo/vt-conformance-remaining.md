...
- [ ] **OSC 110 / 111 / 112: 設定値へのリセット**（設定層待ち）
  - OSC 110 / 111 は今はハードコードされた白 / 黒に戻る。
  - OSC 112 は「未設定」（カーソルをその下のセルの前景色で描く）に戻る。設定に既定のカーソル色が入ったら、その値に戻す。
  - 未設定のとき、`OSC 12 ; ?` は既定の前景色を返すが、実際のカーソルはその下のセルの前景色で描かれる。設定に既定のカーソル色が入ったら、応答と描画の色を一致させる。
- [ ] **OSC 12: fg == bg のセルで、色が未設定のカーソルが見えない**
  - 未設定のカーソルはセルの前景色で塗る（ブロックは前景と背景を入れ替え、bar / underline は帯を前景色で塗る）ので、前景と背景が同じセル（同色の `█`、fg = bg で文字を消しているセル）では 3 形状とも見えない。
  - 以前のピクセル反転では見えていた。Alacritty は前景と背景のコントラストが低いとき、既定の前景色と背景色に切り替える。
  - 明るい背景を明示するプログラムの空白セルは、ほぼ全域がこの状態になる。消去（`EL` / `ED` / `ECH` など）で書く空白セルはペンの背景だけを持ち、前景は `DefaultForeground`（既定は白）になるため。ライトテーマの vim / nvim は空行と行末を `EL` で消すので、そこではカーソルが見えない。背景が純白でなくても（Solarized Light の `#fdf6e3` など）、白いカーソルとのコントラスト比は 1.1 前後しか無い。
  - この場合の塗り色はすでに既定の前景色なので、既定の前景色 / 背景色へのフォールバックでは直らない。塗り色と実際に塗られる背景を比べるガードか、消去セルにペンの前景色を残す VT 側の変更が要る。
  - 設定済みの色にもガードが無い。`OSC 12` で既定の前景色と同じ色を設定すると（未設定時の `OSC 12 ; ?` の応答をそのまま書き戻したときも同じ）、前景が既定色の `SGR 7` のセルの上で 3 形状とも見えない。xterm はカーソル色が前景色と等しいとき、カーソルを背景色で描く。
- [ ] **OSC 11: `padding_color` の黒センチネル**（設定 PR とセット）
  - 既定背景が黒だと fallback 色に倒れるので、明示の `OSC 11;rgb:00/00/00` も同じ扱いになる。
  - この分岐は、グリッドの地・padding 帯・`materialize_default_bg` を通る 2 つのグリフ色（reverse video と、ブロックカーソル下の文字色）を決めている。値で兼用せず、「一度でも設定されたか」を別の信号で持つ。
