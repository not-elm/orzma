...
|---|---|---|
| `XF`, `fe=\E[?1004h`, `fd=\E[?1004l`, `kxIN=\E[I`, `kxOUT=\E[O`（`xterm+focus`。6.0 にはこの entry 自体が無い） | フォーカス通知 | **Tier 2 → Tier 1**。§1-B へ移動済み（2026-09-13） |
...
| `CSI ?1004 h/l` → `CSI I` / `CSI O` | フォーカス通知 | `XF`, `fe`/`fd`, `kxIN`/`kxOUT` | **モードは `VtModes::focus_in_out` に保存されるが送信側が存在しない**（参照は定義と代入の 2 箇所のみ）。**Tier 2 から移動（2026-09-13）** — §0 の版差ノートのとおり ncurses 6.6 の entry が `xterm+focus` を含む。規範文は xterm-ctlseqs.pdf p.58「FocusIn/FocusOut」の *"it causes xterm to send CSI I when the terminal gains focus, and CSI O when it loses focus"*。**参照実装 5 つが同型で、割れている点が無い**（この一覧では珍しい）: tmux の `window_pane_update_focus`（`window.c:672`、ペインごとの `PANE_FOCUSED`）・kitty の `focus_changed`（`screen.c:6640`、`has_focus`）・wezterm の `focus_changed`（`term/src/terminalstate/mod.rs:887`、`self.focused`）・ghostty の `focusCallback`（`Surface.zig:3336`、`Surface.focused`）・alacritty の `on_focus_change`（`input/mod.rs:830`。ペインが無いのでウィンドウ焦点のみ）。**決めた設計 4 つ**: (1) **エッジトリガ** — ペインごとに焦点状態を持ち、変化したときだけ 3 バイトを書く（5/5 が状態と比較して早期 return する）。(2) **モードを立てた時点では送らない** — `CSI ?1004h` は状態を立てるだけで現在の焦点を通知しない（tmux `input.c:2065` は `screen_write_mode_set` のみ、kitty は `SIMPLE_MODE` のみ）。起動時すでに焦点を持っているアプリは `CSI I` を受け取らない。(3) **焦点の条件は「ウィンドウが focused」かつ「アクティブペイン」かつ「オーバーレイがキーボードを持っていない」** — 3 つ目は tmux が `c->overlay_draw == NULL && wp->window->menu == NULL` を要求してメニュー表示中をフォーカス外と扱うのに倣う。orzma では focused webview とピッカーが該当する。**vi モードと IME 合成は焦点喪失にしない**（tmux はコピーモード中もフォーカスを維持する）ので、`KeyboardDisabled` をそのまま流用してはいけない。(4) **置き場所** — `OrzmaTty::send_focus` を `OrzmaTty::send_paste` と同じ形（`bracketed_paste` で門番するのと同様に `focus_in_out` で門番する）で足すが、**`snap_to_live_tail()` は呼ばない**（ユーザー入力ではないのでスクロール追従の対象外）。ウィンドウ焦点は `OrzmuxCommand::WindowFocus { focused: bool }` で backend へ渡し（plain data なので §4 の protocol 純度と両立する）、**ウィンドウ焦点変化・アクティブペイン変化・ペイン生成**の 3 点で再計算する（tmux の `window_set_active_pane` が `lastwp` と `w->active` の両方を更新するのと同じ）。`Window.focused` は `src/input/focus.rs` の `maintain_input_gates` が既に読んでいる。**`[inactive_pane]` との衝突を同時に裁定すること** — 2 つは別レイヤ（nvim は `SGR 48;2;…` でセルの背景色そのものを変え、orzma は shader の `tint_bg` / `dim` を後段に掛ける）なので、上書きではなく合成になる。**既定値では向きが逆で、しかも orzma がほぼ飲む**（`dim = 1.0` は no-op、`tint = 0.85`、`tint_color = #3a3b45` は青灰で**明るい**側）: 実測で nvim の `Normal` bg は `#1a1d23`、`FocusLost` の target は暖色の `#2c2926` だが、線形空間で 0.85 混ぜると tint のみが `#363841`、nvim ありが `#383941` で、**nvim の寄与は 1 チャンネルあたり 2/255**。`dim` を 1.0 未満にしたときだけ本当に二重に暗くなり、そのとき **`dim` は前景グリフにも掛かる**（`tint_bg` は背景だけ）ので効果範囲も食い違う。トリガ集合もズレる: `PaneInactiveStyle` は `on_active_pane_changed` でしか付け外しされないので**ウィンドウ焦点喪失では orzma 側は何も変えない**が、決定 (3) の条件では nvim には `CSI O` が届く。**重複送信には実害がある** — ユーザー設定の `FocusLost` ハンドラは退避先 `vim.g._saved_normal_bg` を無条件に上書きするので、`CSI O` が 2 回続くと退避値が変更後の色になり `FocusGained` で元に戻らなくなる。決定 (1) のエッジトリガはこれを防ぐために必須 | **nvim / vim の `FocusGained` / `FocusLost` が一度も発火しない**。実測（§6 の方法、`TERM=xterm-256color`・`$TMUX` なし）: nvim は `--clean` でも起動時に `CSI ?1004h`・終了時に `?1004l` を無条件で出す。vim は DA2 応答後に出す（orzma の `CSI >0;<ver>;1c` でも出ることを確認。`-u NONE` では termresponse を出さないので沈黙する）。tmux は `focus-events on` のときペイン内が立てた 1004 を `tty_update_mode` で外側へミラーする（既定は off だが、nvim の `:checkhealth` が「`focus-events` is not enabled. `'autoread'` may not work」と警告して on を促す）。落ちる機能は `'autoread'`（外部変更の再読込。AstroNvim は `checktime` を `FocusGained` に付ける）と、`FocusLost` で配色を変えるユーザー設定 |
...
5. **入力側の契約修正**（~~`kbs` の方針決定~~ → ファンクションキー → 修飾キー）。~~Shift-Tab~~ と Insert は **完了（2026-09-11）**。~~Meta~~ は §1-B の `CSI ?1034 h/l` 行のとおり意図的に無視と決着（2026-09-11）。~~`kbs`~~ は §0 の版差ノートにより **要判断が解消（2026-09-13、DEL のまま）**。
   **フォーカス通知（`CSI I` / `CSI O`）もこの層**（orzma が PTY へ書くバイト）に属するので
   ここで実装する。設計は §1-B のフォーカス通知行の (1)–(4) がそのまま使える。項目 10 の
   「非フォーカスで点滅を止める」と項目 11 の非アクティブペインのカーソルは**同じ焦点信号**を
   読むので、3 つまとめて裁定すると設計が 1 回で済む。
