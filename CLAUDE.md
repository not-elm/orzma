...
In-process webview rendering is provided by the external `bevy_cef` crate (a git dependency on the `passthrough` branch, CEF v145 pinned to `145.6.1+145.0.28` in the Makefile). Both the renderer and the helper render process come from `bevy_cef` / `export-cef-dir`; see `make setup-cef`.
