...
- `crates/orzma_vt` (`orzma_vt`) — VT emulation: the `Vt` trait, `OrzmaVt`'s screen/grid/viewport model, CSI/OSC/APC dispatch, selection, vi-cursor state, and the palette/color types. No Bevy or PTY dependency; consumed by `orzma_tty`, `bevy_orzma_mux`, and `orzma_tty_renderer`.
