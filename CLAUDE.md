...
- `crates/orzmux` (`orzmux`) — the built-in multiplexer backend: a Bevy-free thread that owns every pane's PTY-backed `OrzmaTty<OrzmaVt>` and the cell-unit split tree (`layout`), waits on the command channel and each pane's PTY streams with one `crossbeam_channel::Select`, and emits `OrzmuxEvent`s (`Layout` / `Frame` / `Signal` / `PaneOpened` / `SpawnFailed` / `PaneClosed` / `SelectionText`) to the GUI. The GUI holds an `OrzmuxClient` and sends `OrzmuxCommand`s (`Resize`, `NewPane`, `KillPane`, `SelectPane*`, key / mouse / wheel / paste / scroll / selection input). Every type crossing the channel is plain data so the transport can later become a socket to a separate process.
