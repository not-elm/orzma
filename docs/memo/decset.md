...
| Ps | Set | Reset |
| - | - | - |
| 2026 | 同期更新を開く。以後の frame を止める | 同期更新を閉じる。閉じた瞬間の画面を frame にする |

- パースは止めない。止めるのは frame の emit だけで、DSR などの応答と signal は遅れない。
- `Vt::interpret` は同期更新を閉じたバイトで止まり、`consumed` を返す。呼び出し側（`orzma_tty`）は
  その瞬間の frame を取ってから残りを再投入する。閉じた直後に次の同期更新が同じ chunk で始まっても、
  完成した画面を取り損ねない。
